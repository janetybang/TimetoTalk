...
# frequency of observations per cell
xtabs(~ language + activity, freq_all_mm, na.action = na.omit)
xtabs(~ language + activity, mlu_all_mm, na.action = na.omit)
xtabs(~ language + activity, chip_mm, na.action = na.omit)
...
# ref grid for final model
ref_grid(m3_both_tokens_all)


# emmeans
# set REML = T for estimated marginal means
m3_both_tokens_all <- lmer(tokens_permin2 ~ tokens_hr_child + (language * activity) +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = T)
...
tokens
# ggsave("./figures/emmeans_tokens_aug4_2025.pdf", width = 6, height = 6, units = "in")
...
types
# ggsave("./figures/emmeans_types_aug4_2025.pdf", width = 6, height = 6, units = "in")
...
m2_both_mlu_all <- lmer(mlu_w2 ~ tokens_hr_child + language + activity +
                                (1 | id),
                                data = mlu_all_mm, REML = T)

m2_both_mlu_all_emmeans_activity <- emmeans(m2_both_mlu_all, ~ activity)

m2_both_mlu_all_emmeans_activity
pairs(m2_both_mlu_all_emmeans_activity, adjust = "bonferroni")



# emmeans PER language using model WITHOUT interactions - for figure
...
# ggsave("./figures/emmeans_mlu_aug4_2025.pdf", width = 6, height = 6, units = "in")
...
# ggsave("./figures/emmeans_resp_aug4_2025.pdf", width = 6, height = 6, units = "in")
...
# ref grid for final model
ref_grid(m1_both_propimitexp_all)
...
# emmans per language using model WITHOUT interactions
# set REML = T for estimated marginal means
m2_both_propimitexp_all <- lmer(prop_adult_imitexp2 ~ tokens_hr_child + language + activity +
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)
...
# plot - use model 2 to plot emmeans per group
...
# ggsave("./figures/emmeans_imit_exp_aug4_2025.pdf", width = 6, height = 6, units = "in")
...
# ggsave("./figures/emmeans_grid_all_line_aug4_2025.pdf", width = 14, height = 10, units = "in", dpi = 300)

```


# Child Covariate models - all together
```{r}
...
```{r}
# comparing models
m0_both_tokens_all_hi <- lmer(tokens_permin2 ~ tokens_hr_child + activity + 
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

m1_both_tokens_all_hi <- lmer(tokens_permin2 ~ tokens_hr_child + activity + language + 
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

m2_both_tokens_all_hi <- lmer(tokens_permin2 ~ tokens_hr_child + hi + activity + language +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

m3_both_tokens_all_hi <- lmer(tokens_permin2 ~ tokens_hr_child + language + (hi * activity) +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

# see if adding the intx adds
anova(m0_both_tokens_all_hi, m1_both_tokens_all_hi, m2_both_tokens_all_hi, m3_both_tokens_all_hi)
...
```{r}
# comparing models
m0_both_types_all_hi <- lmer(types_permin2 ~ tokens_hr_child + activity +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

m1_both_types_all_hi <- lmer(types_permin2 ~ tokens_hr_child + activity + language +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

m2_both_types_all_hi <- lmer(types_permin2 ~ tokens_hr_child + hi + activity + language +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

m3_both_types_all_hi <- lmer(types_permin2 ~ tokens_hr_child + language + (hi * activity) +
                                (1 | id) + (1 | segment_num),
                                data = freq_all_mm, REML = F)

# see if adding the intx adds
anova(m0_both_types_all_hi, m1_both_types_all_hi, m2_both_types_all_hi, m3_both_types_all_hi)
...
m0_both_mlu_all_hi <- lmer(mlu_w2 ~ tokens_hr_child + activity +
                                (1 | id),
                                data = mlu_all_mm, REML = F)
...
m2_both_mlu_all_hi <- lmer(mlu_w2 ~ tokens_hr_child + hi + activity + language +
                                (1 | id),
                                data = mlu_all_mm, REML = F)

m3_both_mlu_all_hi <- lmer(mlu_w2 ~ tokens_hr_child + language + (hi * activity) +
                                (1 | id),
                                data = mlu_all_mm, REML = F)

# see if adding the intx adds
anova(m0_both_mlu_all_hi, m1_both_mlu_all_hi, m2_both_mlu_all_hi, m3_both_mlu_all_hi)
...
```{r}
m0_both_propresp_all_hi <- lmer(prop_adultresp2 ~ tokens_hr_child + activity +
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

m1_both_propresp_all_hi <- lmer(prop_adultresp2 ~ tokens_hr_child + activity + language + 
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

m2_both_propresp_all_hi <- lmer(prop_adultresp2 ~ tokens_hr_child + hi + activity + language + 
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

m3_both_propresp_all_hi <- lmer(prop_adultresp2 ~ tokens_hr_child + (hi * activity) + language + 
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

# see if adding the intx adds
anova(m0_both_propresp_all_hi, m1_both_propresp_all_hi, m2_both_propresp_all_hi, m3_both_propresp_all_hi)
```
...
m0_both_propimitexp_all_hi <- lmer(prop_adult_imitexp2 ~ tokens_hr_child + activity +
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

m1_both_propimitexp_all_hi <- lmer(prop_adult_imitexp2 ~ tokens_hr_child + activity + language + 
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

m2_both_propimitexp_all_hi <- lmer(prop_adult_imitexp2 ~ tokens_hr_child + hi + activity + language + 
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

m3_both_propimitexp_all_hi <- lmer(prop_adult_imitexp2 ~ tokens_hr_child + (hi * activity) + language + 
                                (1 | id) + (1 | segment_num),
                                data = chip_mm, REML = F)

# see if adding the intx adds
anova(m0_both_propimitexp_all_hi, m1_both_propimitexp_all_hi, m2_both_propimitexp_all_hi, m3_both_propimitexp_all_hi)
